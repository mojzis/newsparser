...
class ArticleEvaluation(AnalyticsBase):
    ...
    summary: str = Field(
        ..., min_length=10, max_length=500, description="Brief summary"
    )
    perex: str = Field(
        ...,
        min_length=10,
        max_length=200,
        description="Witty, engaging summary for display",
    )
    ...
    content_type: str = Field(
        ...,
        description="Type of content (video, newsletter, article, blog post, product update, invite)",
    )
    language: str = Field(
        ..., description="Language of the content (e.g., en, es, fr, ja)"
    )
    ...
    published_date: datetime | None = Field(None, description="Article publish date")
