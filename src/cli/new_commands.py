...
def cli() -> None:
    ...
    try:
        ...
        if not config_manager.validate_config():
            console.print(
                "[red]Configuration validation failed. Please check your config files.[/red]"
            )
            raise click.Abort()
...
@click.option(
    "--days-back",
    default=7,
    help="Days to look back for unfetched URLs and unevaluated content (default: 7)",
)
@click.option(
...
def run_all(
    ...
    ctx.invoke(
        ...
        regenerate_evaluations=regenerate_evaluations,
        export_parquet=export_parquet,
        expand_references=expand_references,
        max_reference_depth=max_reference_depth,
        sitemap=sitemap,
